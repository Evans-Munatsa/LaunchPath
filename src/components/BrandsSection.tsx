          ...
          {cases.map((c, i) => (
            <Reveal as="article" key={i} delay={i * 100}>
              <div className="group relative overflow-hidden rounded-3xl hover-lift" style={{ aspectRatio: "3/4" }}>
                <img src={c.img} alt={c.title} loading="lazy" width={768} height={1024} className="image-zoom absolute inset-0 h-full w-full object-cover" />
                ...
                <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-3">
                  ...
                  <Link to="/about-us" className="shrink-0 rounded-full bg-white px-5 py-2 text-[13px] font-semibold text-foreground transition-transform duration-300 group-hover:scale-105">
                    Read
                  </Link>
                </div>
              </div>
